...
class LaplaceSumQuery(dp_query.SumAggregationDPQuery):
  """Implements DPQuery interface for Laplace sum queries.

  Clips records to bound the L2 norm, then adds Laplace noise to the sum.
  """
  ...
  def get_noised_result(self, sample_state, global_state):
    ...
    if distutils.version.LooseVersion(
      ...
      def add_noise(v):
        return v + np.random.laplace(loc=0.0,
            size=tf.shape(input=v), scale=global_state.stddev/np.sqrt(2))
    else:
      def add_noise(v):
        return v + tf.cast(np.random.laplace(loc=0.0, size=tf.shape(input=v), 
            scale=global_state.stddev/np.sqrt(2)), dtype=v.dtype)
    ...
    event = dp_event.LaplaceDpEvent(noise_multiplier)
