...
def repeat_logarithmic_rdp(orders, rdp, gamma):
	...
	rdp_out = [None]*n
	for i in range(n):
		if orders[i] == 1: continue # unfortunately the formula doesn't work in this case
		for j in range(n):
			# Compute (orders[i],eps)-RDP bound on A_gamma given that Q satisfies (orders[i],rdp[i])-RDP and (orders[j],rdp[j])-RDP
			eps = rdp[i] + (1-1/orders[j])*rdp[j] + math.log(1/gamma-1)/orders[j] + (math.log(1/gamma-1)-math.log(math.log(1/gamma)))/(orders[i]-1)
...
def repeat_geometric_rdp(orders, rdp, gamma):
	...
	rdp_out = [None]*n
	for i in range(n):
		if orders[i] == 1: continue # formula doesn't work in this case
		for j in range(n):
			eps = rdp[i] + 2*(1-1/orders[j])*rdp[j] + (2/orders[j]+1/(orders[i]-1))*math.log(1/gamma)
			if rdp_out[i] is None or eps < rdp_out[i]:
				rdp_out[i] = eps
	return rdp_out
...
def repeat_negativebinomial_rdp(orders, rdp, gamma, eta):
	...
	for i in range(n):
		if orders[i] == 1: continue # forumla doesn't work for lambda=1
		for j in range(n):
			eps = rdp[i]+(1+eta)*(1-1/orders[j])*rdp[j]-((1+eta)/orders[j]+1/(orders[i]-1))*math.log(gamma)+foo/(orders[i]-1)+(1+eta)*math.log1p(-gamma)/orders[j]
			if rdp_out[i] is None or eps < rdp_out[i]:
				rdp_out[i] = eps
	return rdp_out
...
if load_values_to_reproduce_paper_fig:
	...
	gammas = np.asarray([])  # 1D array of gamma parameters to the random distributions.
...
for dist_id in range(4):
	...
	if dist_id == 0:
		plt.hlines(res_y_max[0], xmin=-1., xmax=20., color='r', label='baseline (non-private search)')
	if dist_id >= 1:
		res_x = res_x[2:]
		res_y = res_y[2:]
