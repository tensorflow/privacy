...
def decay(cls: int | np.ndarray, max_samples: int, num_classes: int, ratio: float = 10):
...
def scores_single(
    ...
    """Score Calculation for a single public sample. The score is calculated as the sum
    of the clipped pairwise distances between the public sample and the private samples.
...
def scores_multiple(
    ...
    Args:
        ...
        max_score (float, optional): maximum score (in (min_score, 2]). Defaults to 2.0.
        batch_size_y (int, optional): batch size (impacts VRAM usage). Defaults to 5000.
