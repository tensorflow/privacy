...
class SparseNoiseUtilsTest(tf.test.TestCase, parameterized.TestCase):
  ...
  def test_split_noise_multiplier(
    ...
    total_noise_multiplier_sparse = (
        1.0 / (num_sparse_layers * 1.0 / noise_multiplier_sparse**2) ** 0.5
    )
  ...
  def test_split_noise_multiplier_errors(
    ...
    with self.assertRaises(AssertionError) as cm:
      _ = sparse_noise_utils.split_noise_multiplier(
          noise_multiplier,
          sparse_selection_ratio,
          sparse_selection_contribution_counts,
      )
  ...
  def test_sample_sparse_indices_one_prob(self, max_index):
    sampled_indices = (
        sparse_noise_utils.sample_sparse_indices(max_index, 1.0)
        .numpy()
        .tolist()
    )
    expected_indices = list(range(max_index + 1))
    self.assertListEqual(sampled_indices, expected_indices)
  ...
  def test_sample_sparse_indices_zero_prob(self, max_index):
    sampled_indices = (
        sparse_noise_utils.sample_sparse_indices(max_index, 0.0)
        .numpy()
        .tolist()
    )
  ...
  @parameterized.named_parameters(
      dict(
          testcase_name='max_index_100_prob_5',
          prob=0.5,
          max_index=10,
      ),
      dict(
          testcase_name='max_index_100_prob_1',
          prob=0.25,
          max_index=20,
      ),
      dict(
          testcase_name='max_index_100_prob_9',
          prob=0.75,
          max_index=20,
      ),
  )
  def test_sample_sparse_indices_random(self, max_index, prob):
    sampled_indices = sparse_noise_utils.sample_sparse_indices(max_index, prob)
    ...
    for _ in range(num_trials):
      sampled_indices = sparse_noise_utils.sample_sparse_indices(
          max_index, prob
      ).numpy()
    ...
    for i in range(max_index):
      self.assertGreater(
          stats.binomtest(k=histogram[i].item(), n=num_trials, p=prob).pvalue,
          1e-10,
      )
  ...
  def test_sample_indices_empty(self):
    ...
    threshold = 2
    sampled_indices = sparse_noise_utils.sample_indices(
  ...
  def test_sample_indices_without_noise(self):
    contribution_counts = tf.SparseTensor(
        indices=[[0], [3], [5], [7]],
  ...
  def test_sample_indices_with_noise(self):
    contribution_counts = tf.SparseTensor(
        indices=[[0], [3], [5], [7]],
  ...
  def test_add_sparse_gradient_noise(self):
    ...
    self.assertListEqual(noised_grad.indices.numpy().tolist(), expected_indices)
    expected_zero_index_values = tf.ones((2,)).numpy().tolist()
    zero_index_values = noised_grad.values[0].numpy().tolist()
    self.assertListEqual(zero_index_values, expected_zero_index_values)
    one_index_values = noised_grad.values[1].numpy().tolist()
  ...
  def test_get_contribution_counts(self):
    ...
    self.assertListEqual(contribution_counts, expected_contribution_counts)
